...
COMPOSITION_CX_TABLE = {"cz":1, "cy":1, "swap":3, "ch":2, "ccx":6, "cswap":8, "crx":2, "cry":2,
        ...
        "c4x":18, "cu":2, "cp":2, "ecr": 0, "rzx":0, "majority": 8, "unmaj" : 8, "add4" : 1}
...
function_table = {"gate":1, "param0":0, "rzx":0}
# Format: {"function_name": cx_gate_num}
cx_table = {"gate":1, "param0":0, "rzx":0}

#Keywords in QASM that are currently not used
other_keys = ["measure", "barrier", "OPENQASM", "include", "creg", "if", "reset", "gate", "{", "}"]
...
def paramlist_to_ga(param_list):
    ...
    if param_list.find(')') != -1: 
        ...
        for expr in gate_param:
            # this trick is supposed to permit to not have problem with general param0
            # arguments
            slist[0] +=  "param0"

            # this line is looking for some pi expression in the parameters, the problem
            # is that: if you have any sort of different parameters you will end up having
            # errors
            #slist[1] +=  str(eval(expr, {'pi':cmath.pi})) + ", "
...
def main(source_file):
    ...
    print ("== SV-Sim: Translating " + input_file + " to " + args.output + " ==")
